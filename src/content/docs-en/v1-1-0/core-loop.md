...
The `AgentLoop` is the heart of the system. See also [`logging.md`](/architect-docs/en/docs/v1-1-0/logging) for details on the logging system.
...
As of v1.0.0, the `build` agent also has access to `dispatch_subagent` as an additional tool, which allows delegating sub-tasks to agents with isolated context. See [`tools-and-execution.md`](/architect-docs/en/docs/v1-1-0/tools-and-execution) for details.

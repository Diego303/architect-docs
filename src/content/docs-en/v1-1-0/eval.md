...
Starting with v1.1.0, the competitive evaluation emits HUMAN-level events that produce clear visual output on stderr. The user can see the ranking in real time without needing `-v` flags.
...
Final ranking: gpt-4.1 > claude-sonnet > gpt-4.1-mini
```

Models in positions 1, 2, and 3 show medals. From the 4th position onward, `#N` is shown.

### Emitted events

| Event | When | Data |
|-------|------|------|
| `competitive.model_done` | On completing each model | model, rank, score, cost, checks_passed, checks_total |
| `competitive.ranking` | When the evaluation finishes | ranking (list of {model, score, rank}) |

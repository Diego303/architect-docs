...
guardrails:
  enabled: true
  # Blocks read AND write (secrets the LLM should never see)
  sensitive_files: [".env*", "*.pem", "*.key"]
  # Blocks write only (the LLM can read but not modify)
...
Sessions and reports:
  ...
  --report-file PATH        Save report to file (format inferred from extension)
...
Notable env vars:
  ARCHITECT_LANGUAGE         Language: en (default) | es
...
By default, all system messages (logs, prompts, reports) are in **English**. To switch to Spanish:
...
See [`i18n.md`](/architect-docs/en/docs/v1-1-0/i18n) for full details on what changes and what does not.
...
```yaml
language: en               # "en" (default) | "es" — system message language
...
guardrails:
  enabled: true
  sensitive_files: [".env*", "*.pem", "*.key"]  # blocks read + write
  protected_files: ["config/production.yaml"]     # blocks write only
  quality_gates:
    - name: tests
      command: "pytest tests/ -x"
      required: true

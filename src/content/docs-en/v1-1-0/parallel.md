...
Starting with v1.1.0, the parallel execution emits HUMAN-level events that produce clear visual output on stderr. The user can see the result of each worker without needing `-v` flags.

```
   + Worker 1 (gpt-4.1) -> success ($0.0456, 120.3s)
   + Worker 2 (claude-sonnet) -> success ($0.0312, 98.7s)
   x Worker 3 (gpt-4.1-mini) -> failed ($0.0089, 45.1s)
...
| Event | When | Data |
|-------|------|------|
| `parallel.worker_done` | On completing each worker | worker, model, status, cost, duration |
| `parallel.worker_error` | If a worker fails with an exception | worker, error |
| `parallel.complete` | When all workers finish | total_workers, succeeded, failed, total_cost |

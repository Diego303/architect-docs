...
## YAML Validation (v1.1.0)

Before executing, the pipeline fully validates the YAML. If there are errors, a descriptive message is shown and the process exits with exit code 3 (`CONFIG_ERROR`) without running any step.

### Validation rules

| Rule | Description |
|------|-------------|
| `prompt` required | Each step must have a non-empty `prompt` field |
| Valid fields | Only allowed: `name`, `agent`, `prompt`, `model`, `checkpoint`, `condition`, `output_var`, `checks`, `timeout` |
| Unknown fields | Rejected with error. The `task` field includes hint: "did you mean `prompt`?" |
| At least 1 step | The pipeline must have at least one step defined |
| Step format | Each step must be a YAML object (dict), not a string or other type |
| Prompt not whitespace-only | A prompt with only spaces or newlines is rejected |
...
steps:
  - name: analyze
    task: "Analyze the project"    # <- incorrect field
  - name: implement
    prompt: ""                      # <- empty prompt
  - name: deploy
    prompt: "Deploy"
...
```bash
$ architect pipeline bad-pipeline.yaml
Validation error: Pipeline 'bad-pipeline.yaml' has validation errors:
  analyze: unknown field 'task' (did you mean 'prompt'?)
  analyze: missing 'prompt' (the field 'task' is not valid, use 'prompt')
  implement: missing 'prompt' or empty
  deploy: unknown field 'priority'
```

All errors are collected and shown together in a single message, making correction easier.
...
`PipelineValidationError` inherits from `ValueError` for backward compatibility.

---

## Visual output (HUMAN logging)

Starting with v1.1.0, Pipeline Mode emits HUMAN-level events that produce clear visual output on stderr. The user can see the progress of each step in real time without needing `-v` flags.
...
━ Pipeline step 2/3: implement (agent: build) ━━━━━━━━━━━━━━━
   ...
   ⏭️  Step 'deploy' skipped (condition not met)
```
...
| Event | When | Data |
|-------|------|------|
| `pipeline.step_start` | At the start of each step | step, agent, index, total |
| `pipeline.step_skipped` | When the condition is not met | step |
| `pipeline.step_done` | When each step completes | step, status, cost, duration |
...
architect pipeline workflow.yaml --var feature="auth"
  ...
  │       ├── yaml.safe_load(file)
  │       ├── _validate_steps(steps_data) → PipelineValidationError if errors
  │       ├── Merge variables YAML + CLI
  │       └── Build PipelineConfig with steps
  │
  ├── 2. runner.run(from_step=None, dry_run=False)
  │       │
  │       ├── For each step:
  │       │   ├── 2a. _eval_condition(condition) → skip if False
  │       │   ├── 2b. _resolve_vars(prompt) → substitute {{variables}}
  │       │   ├── 2c. agent_factory(agent=step.agent, model=step.model)
  │       │   │       └── Fresh AgentLoop with ContextBuilder, CostTracker, etc.
  │       │   ├── 2d. agent.run(resolved_prompt) → AgentState
  │       │   ├── 2e. If output_var: variables[output_var] = state.final_output
  │       │   ├── 2f. If checks: _run_checks(checks) → checks_passed
  │       │   ├── 2g. If checkpoint: _create_checkpoint(step_name)
  │       │   └── 2h. Record PipelineStepResult
  │       │
  │       └── Return list[PipelineStepResult]
  │
  └── 3. Display results
```

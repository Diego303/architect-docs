...
Starting with v1.1.0, the Ralph Loop emits HUMAN-level events that produce clear visual output on stderr. The user can see the progress of each iteration in real time without needing `-v` flags.

```
--- Ralph iteration 1/5 (check: pytest tests/) ----------------------
   Checks: 3/5 passed
   x Iteration 1 -> partial ($0.0234, 45.2s)
...
| Event | When | Data |
|-------|------|------|
| `ralph.iteration_start` | At the start of each iteration | iteration, max_iterations, check_cmd |
| `ralph.checks_result` | After running the checks | iteration, passed, total, all_passed |
| `ralph.iteration_done` | On completing each iteration | iteration, status, cost, duration |
| `ralph.complete` | When the loop finishes (any reason) | total_iterations, status, total_cost |

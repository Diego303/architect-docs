...
# Only --report-file: format is inferred from extension (v1.1.0)
architect run "task" --mode yolo --report-file report.json    # -> json
architect run "task" --mode yolo --report-file report.md      # -> markdown
architect run "task" --mode yolo --report-file pr.html        # -> github
```
...
| `--report-file PATH` | Save report to file. If `--report` is not specified, the format is inferred from the extension: `.json` -> json, `.md` -> markdown, `.html` -> github (default: markdown). Parent directories are automatically created if they do not exist. |
...
- **CLI**: `--report` and `--report-file` flags in `src/architect/cli.py` (includes `_infer_report_format()` for extension inference and `_write_report_file()` for robust writing with directory creation)
- **Tests**: `tests/test_reports/` (34 tests: 20 original + 8 inference + 5 writing + 1 fix) + `scripts/test_phase_b.py` section B2 (8 tests, 24 checks)

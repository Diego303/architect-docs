...
description: "22+ defensive layers, threat model, path traversal, sensitive files, command security, hardening."
...
|-------|--------|-------------------|
| Adversarial LLM / hallucination | The model attempts to read `/etc/passwd`, execute `rm -rf /`, or escape the workspace | Path traversal prevention + blocklist + workspace sandboxing + sensitive_files (v1.1.0) |
...
See [`containers.md`](/architect-docs/en/docs/v1-1-0/containers) for complete Containerfiles.
...
| 22 | Sensitive file protection | `guardrails.py` | Block read+write of files with secrets (`sensitive_files`) — prevents the LLM from reading `.env`, `*.pem`, `*.key` (v1.1.0) |

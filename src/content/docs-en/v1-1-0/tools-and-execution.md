...
| Check | Method | When |
|-------|--------|------|
| Sensitive files | `check_file_access()` | On **all** filesystem tools (read, write, edit, delete) -- blocks reading and writing of `sensitive_files` |
| Protected files | `check_file_access()` | On write tools (write, edit, delete, patch) -- blocks only writing of `protected_files` |
| Blocked commands | `check_command()` | On `run_command` -- includes shell read detection (`cat`, `head`, `tail`) for sensitive files |

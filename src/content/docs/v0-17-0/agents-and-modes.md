...
Los hooks del lifecycle (`HookExecutor`) y los guardrails (`GuardrailsEngine`) se integran en el `ExecutionEngine`, no en el system prompt. Los guardrails se evalúan antes de cada tool call, y los hooks pre/post se ejecutan alrededor de cada acción. Ver [Tools y Ejecución](/architect-docs/docs/v0-17-0/tools-and-execution) para el pipeline completo de 10 pasos.
